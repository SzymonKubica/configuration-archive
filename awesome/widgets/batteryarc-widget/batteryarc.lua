...
local function worker(user_args)
    ...
    local medium_level_color = args.medium_level_color or '#d74f00'
    local charging_color = args.charging_color or '#03fcf0'

    local warning_msg_title = args.warning_msg_title or 'Power level critical'
    local warning_msg_text = args.warning_msg_text or 'Battery is dying'
    local warning_msg_position = args.warning_msg_position or 'top_right'
    local warning_msg_icon = args.warning_msg_icon or WIDGET_DIR .. '/low_battery.png'
    ...
    elseif show_notification_mode == 'on_click' then
        batteryarc_widget:connect_signal('button::press', function(_, _, _, button)
            if (button == 1) then toggle_notification() end
        end)
    end
